...
async function main() {
  ...
  try {
      ...
      console.error("Directory examples/basic-server/src/plugins/ not found.");
      process.exit(1);
  }
  ...
  const importLines = foundPlugins.map(p => `import { ${p.import} } from "./examples/basic-server/src/plugins/${p.name}";`).join("\n");
...
import { RouteBuilder, MiddlewareBuilder } from "./src/router";
