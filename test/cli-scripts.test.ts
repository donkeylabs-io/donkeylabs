...
const TEST_DIR = join(PROJECT_ROOT, "test", ".temp-cli-tests");
...
describe("CLI Scripts", () => {
  ...
  describe("generate-registry.ts", () => {
    it("should generate registry files", async () => {
      ...
      expect(result).toBe(0);
      expect(existsSync(join(PROJECT_ROOT, "registry.d.ts"))).toBe(true);
      expect(existsSync(join(PROJECT_ROOT, "registry.ts"))).toBe(true);
    });

    it("should include plugin information in registry", async () => {
      await Bun.spawn(["bun", join(SCRIPTS_DIR, "generate-registry.ts")]).exited;

      const registryDts = await readFile(
        join(PROJECT_ROOT, "registry.d.ts"),
        "utf-8"
      );

      // Should include existing plugins
      expect(registryDts).toContain("PluginRegistry");
      expect(registryDts).toContain("auth");
    });
  });
...
describe("Script Package.json Integration", () => {
  ...
  it("should run gen:registry via npm script", async () => {
    const result = await Bun.spawn(["bun", "run", "gen:registry"]).exited;
    expect(result).toBe(0);
  });
